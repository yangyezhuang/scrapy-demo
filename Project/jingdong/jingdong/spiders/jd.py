...
class JdSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for li in lists:
            ...
            item['id'] = id
            item['name'] = li.xpath('.//div[@class="p-name p-name-type-2"]/a/em/text()[1]').extract_first().split(' ')[0]
            info_url = f'https://item.jd.com/{id}.html'
            # item['url'] = info_url
            item['price'] = li.xpath(f'.//strong[@class="J_{id}"]/i/text()').extract_first('')
            # item['store'] = li.xpath('.//div[@class="p-shop"]//a/text()').extract_first()
    ...
    def comment_parse(self, response):
        ...
        item['good'] = good_comment
        item['bad'] = bad_comment
        yield item
