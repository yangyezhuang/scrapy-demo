...
from ..items import CarsafetyItem


class CiasiSpider(scrapy.Spider):
    name = 'ciasi'
    allowed_domains = ['www.ciasi.org.cn']
    start_urls = ['https://www.ciasi.org.cn/Home/safety/index?sid=15&bid=&cid=&sss=1&year=51,50']

    def parse(self, response):
        lists = response.xpath('//div[@class="eval_it_body"]/div')
        for li in lists:
            item = CarsafetyItem()
